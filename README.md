...
The Icicle Kit supports booting a Linux image from an external QSPI flash memory. The list of officially supported QSPI flash memory devices are shown below:

| Manufacturer | Part No.     | Type | Density       |
|--------------|--------------|------|---------------|
| Winbond      | W25N01GVZEIG | NAND | 1Gb (128MB)   |
| Micron       | MT25QL256    | NOR  | 256Mb (32MB)  |

The listed flash memories can be connected to the Icicle Kit using commercially available expansion boards and Raspberry Pi HATs using the Raspberry Pi 4 Interface (J26) on an Icicle Kit.
...
The `core-image-minimal-mtdutils` generates a Linux image with either a `.nand.mtdimg` or `.nor.mtdimg` file extension in the deploy directory. This image is suitable for programming to a flash memory device.

##### Building a Linux image suitable for a NAND flash memory (default)
When building `core-image-minimal-mtdutils`, images suitable for programming to a Winbond NAND flash memory will be generated by default.
...
##### Building a Linux image suitable for a NOR flash memory

To generate an image for the Micron NOR flash memory, the following setting must be uncommented in the Icicle Kit machine configuration file (conf/machine/icicle-kit-es.conf):
...
For flashing a Linux image suitable for a **NAND** flash memory:
```bash
$ sudo dd if=tmp-glibc/deploy/images/icicle-kit-es/core-image-minimal-mtdutils.nand.mtdimg of=/dev/sdX
```

For flashing a Linux image suitable for a **NOR** flash memory:
```bash
$ sudo dd if=tmp-glibc/deploy/images/icicle-kit-es/core-image-minimal-mtdutils.nor.mtdimg of=/dev/sdX
```
...
| `core-image-minimal-mtdutils`     | A small image with minimal MTD utilities suitable for programming to flash NAND and NOR devices |
